...
## 🧠 Skills System — Persistent Memory

After every scan, Hermes-4-405B produces a security rule in the `skill_learned` field. This rule is written to `/var/www/html/hermes-skills.json` via `tg_bot.py`. The last 50 skills are retained. Skills are also appended to `/root/.hermes/memories/MEMORY.md`.

The `/api/skills` GET endpoint returns all skills. Displayed grouped by domain in the Schedule & Skills tab on the frontend.
