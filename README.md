...
No budget. No shortcuts. Just determination.

All the code was written using **Claude's free tier**. When I hit the daily limit, I needed more. So I found 10 friends, asked each one for their phone number, and opened 10 separate Claude accounts for verification — one by one — just to keep writing code.

For the AI API, Nous Research gave me **$10 in credits**. That's it. The entire backend — every scan, every agent call, every arena battle — runs on that.

For the server, I used Linode's $100 credit and deployed on a machine that already runs a **Republic blockchain validator** — a real production node, now also running Hermes Security 24/7.

**$0 out of pocket. Maximum determination.**

This tool is live at the link below — not a demo, not a mock. A real server, scanning real domains, right now.
...
## ⚔️ Security Arena — 3-Agent Adversarial Analysis

The `/api/arena` endpoint launches **BREACH**, **SENTINEL**, and **ARBITER** — three specialized Hermes-4-405B agents — simultaneously against the same domain using `threading.Thread`.

- 🔴 **BREACH** — Offensive agent. Thinks like an attacker. Hunts for exploitable weaknesses, scores from a threat perspective, returns `findings`, `score`, `verdict`, `top_threat`.
- 🔵 **SENTINEL** — Defensive agent. Evaluates existing protections, scores from a defense perspective, maps mitigations to each threat.
- 🟠 **ARBITER** — Judge agent. Waits for both BREACH and SENTINEL to finish, then cross-examines their findings. Returns `final_verdict`, `consensus_score`, `winner`, `red_valid_points`, `blue_valid_points`, `priority_fix`.

Results arrive with **animated score count-up** and **SVG ring fill** for each agent — staggered at 500ms / 650ms for cinematic reveal. Winner badge animates in with a pop effect.

**Arena Leaderboard** — Every run is appended to `hermes-arena-leaderboard.json` on the server and displayed in a live table.

---

## 💬 4-Agent Security Debate — Real-Time Streaming

The `/api/arena/debate/stream` endpoint runs a **Server-Sent Events (SSE)** stream. Four agents debate a specific security finding in sequence, turn by turn:

1. 🟡 **SCRIBE** — Researcher. Searches for relevant CVEs, OWASP references, academic citations. Provides evidence base.
2. 🔴 **BREACH** — Attacker. Argues why the finding is critically exploitable. Pushes back on SENTINEL's mitigations.
3. 🔵 **SENTINEL** — Defender. Argues why existing controls reduce risk. Pushes back on BREACH's attack chains.
4. 🟠 **ARBITER** — Judge. Intervenes mid-debate to correct logical errors. Delivers final verdict with `consensus_score`, `winner`, `risk_level`, `priority_fix`.

**Frontend features:**
- **Typewriter effect** — each agent's message streams word-by-word at 18ms intervals
- **Debate Log side panel** — compact avatar-bubble timeline (SC / BR / SN / AR initials) updates in real-time alongside the main chat
- **Skills learned** — each agent can emit a `skill_learned` field mid-debate; collected and displayed in a panel below
- **Turn counter** — live "N turns" badge updates as debate progresses
- **Export** — after debate ends, ⬇ JSON and ⬇ PDF buttons appear. PDF is dark-themed, agent-color-coded, includes CVE refs and final verdict block.
...
## 🧠 Persistent Memory — Skills That Learn Over Time

After every scan, Hermes-4-405B extracts a `skill_learned` field — a one-sentence distillation of the most important security insight from that scan. This is:

- Appended to `hermes-skills.json` (domain + skill + timestamp, last 50 entries)
- Written to `/root/.hermes/memories/MEMORY.md` for cross-session persistence
- Displayed in the frontend after each scan and debate
...
**Scan PDF** — jsPDF library inside `hermes-addons.js`. Dark theme: background `#040c10`, header bar `rgba(0,255,136,0.15)`, score color dynamic. Score ring drawn as SVG. Vuln counts read from DOM via `data-hermes-{level}` attributes. Log lines color-coded by OK/WARN/ERROR status. File name auto-generated as `hermes-report-{domain}-{timestamp}.pdf`.

**Debate PDF** — Generated after debate completes. Dark-themed A4 layout, per-agent color-coded badges (SCRIBE amber / BREACH red / SENTINEL blue / ARBITER purple), CVE references inline, skill_learned notes, final verdict block with consensus score and priority fix. File name: `hermes-debate-{domain}-{timestamp}.pdf`.

**Debate JSON** — Full structured export: domain, timestamp, all turns (agent / role / turn / message / refs / skill), final verdict object. Available immediately after debate ends via ⬇ JSON button.
...
PDF         → jsPDF (browser-side, scan + debate export)
...
Memory      → hermes-skills.json + MEMORY.md (persistent skill storage)
Arena       → Threading + SSE streaming (BREACH / SENTINEL / ARBITER / SCRIBE)
```
...
## 🔥 Cyber Roast — AI-Powered Security Roast Engine

A dedicated tab where Hermes-4-405B roasts your security mistakes — brutally, hilariously, and with real fixes.

**How it works:**

Every message goes directly to Hermes-4-405B via `hermes_chat()` — not a prompt injection, a real API call with a dedicated `CYBERROAST_SYSTEM` prompt and full conversation history. The model always responds with two parts:

- **😤 Roast** — 3-5 sentences of savage, profane, laugh-out-loud burns tailored to the specific mistake
- **⚕ Real Fix** — Concrete, pro-level security advice with code snippets (Python, nginx, bash)

**Key behaviors:**
- Security problems → roasts the mistake hard, then provides expert fix
- Casual messages ("thanks", "hi", "lol") → roasts you for wasting time, ties it back to security
- Every reply has a different opener — never the same "Jokes aside, bro" twice
- Full conversation history sent with each request — context is never lost
- Typewriter streaming effect — responses appear character by character like ChatGPT

**Technical details:**
- Endpoint: `POST /api/cyberroast` — accepts `question` + `history[]`
- System prompt: `CYBERROAST_SYSTEM` — separate from all other Hermes endpoints
- Model: Hermes-4-405B with `temperature: 0.7` for consistent JSON output
- Parse logic: 4-layer fallback — direct JSON → regex object match → key extraction → raw text
- Frontend: vanilla JS typewriter at 14ms/char (roast) and 10ms/char (fix)

**Example openers the model generates:**
> *"Alright, let's unfuck this situation:"*
> *"Here's your intervention, my guy:"*
> *"Since you clearly need an adult in the room:"*
> *"Ok real talk, here's how you stop the bleeding:"*
> *"Before you get pwned into oblivion:"*
